...
#include "infers/vp_yunet_face_detector_node.h"
#include "infers/vp_sface_feature_encoder_node.h"
#include "osd/vp_face_osd_node_v2.h"
...
// Enumerations
enum vp_nodes::vp_node_type { SRC, DES, MID };
enum vp_nodes::vp_infer_type { PRIMARY, SECONDARY };
...
// Factory functions to create nodes using std::make_shared
namespace vp_nodes {
    // Node creation functions
    %inline %{
        std::shared_ptr<vp_file_src_node> create_file_src_node(const std::string& node_name, 
                                                               int channel_index, 
                                                               ...
                                                               int skip_interval = 0) {
            return std::make_shared<vp_file_src_node>(node_name, channel_index, file_path, resize_ratio, cycle, gst_decoder_name, skip_interval);
        }
        
        std::shared_ptr<vp_screen_des_node> create_screen_des_node(const std::string& node_name, 
                                                                   int channel_index, 
                                                                   bool osd = true,
                                                                   const vp_objects::vp_size& display_w_h = {}) {
            return std::make_shared<vp_screen_des_node>(node_name, channel_index, osd, display_w_h);
        }
        
        std::shared_ptr<vp_yunet_face_detector_node> create_yunet_face_detector_node(const std::string& node_name, 
                                                                                    const std::string& model_path, 
                                                                                    float score_threshold = 0.7, 
                                                                                    float nms_threshold = 0.5, 
                                                                                    int top_k = 50) {
            return std::make_shared<vp_yunet_face_detector_node>(node_name, model_path, score_threshold, nms_threshold, top_k);
        }
        
        std::shared_ptr<vp_sface_feature_encoder_node> create_sface_feature_encoder_node(const std::string& node_name, 
                                                                                        const std::string& model_path) {
            return std::make_shared<vp_sface_feature_encoder_node>(node_name, model_path);
        }
        
        std::shared_ptr<vp_face_osd_node_v2> create_face_osd_node_v2(const std::string& node_name) {
            return std::make_shared<vp_face_osd_node_v2>(node_name);
        }
    %}
