...
int main(int argc,char** argv) {
    ...
    auto inputNet = cv::dnn::readNet("./models/openpose/pose/coco_pose_iter_440000.caffemodel", "./models/openpose/pose/coco_pose_deploy.prototxt");
    ...
    //cv::Mat input = cv::imread("./14.jpg");
    
    while(capture.read(input)) {
    
        std::chrono::time_point<std::chrono::system_clock> startTP = std::chrono::system_clock::now();

        cv::Mat inputBlob = cv::dnn::blobFromImage(input, 1.0/255.0, cv::Size(368, 368), cv::Scalar(0,0,0), false, false);
